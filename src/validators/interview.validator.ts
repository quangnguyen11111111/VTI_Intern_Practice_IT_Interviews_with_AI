...
const taxonomyReferenceSchema = z
  ...
  .refine((value) => !/[\u0000-\u001F\u007F]/.test(value), 'Giá trị taxonomy chứa ký tự không hợp lệ');
...
const uniqueTaxonomyReferences = z
  ...
  .refine((values) => new Set(values).size === values.length, 'Danh sách công nghệ không được trùng lặp');
