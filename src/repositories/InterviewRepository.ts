...
export class InterviewRepository implements IInterviewRepository {
  ...
  private mapSessionToEntity(sessionDoc: any, questionsDoc: any[]): InterviewEntity {
    return {
      ...
      rubricVersion: sessionDoc.rubricVersion ?? 1,
      submissionVersion: sessionDoc.submissionVersion ?? 0,
      activeOperationId: sessionDoc.activeOperationId?.toString?.() ?? null,
      safeErrorCode: sessionDoc.safeErrorCode ?? null,
  ...
  private mapQuestionToEntity(questionDoc: any): InterviewQuestionEntity {
    return {
      ...
      answerVersion: questionDoc.answerVersion ?? 0,
