...
- Include questions about mentoring, code review strategy, and technical leadership.`;
    ...
    // Default fallback if level is unrecognized
    // (e.g. an ID was passed instead of a string)
...
- Make sure to keep the questions balanced.`;
  ...
  async generateQuestions(
    ...
    const levelGuidance =
      setupData.level
        ? this.getLevelGuidance(
            setupData.level
          )
        : `LEVEL GUIDANCE (From JD):
...
- Focus: Ask practical questions that evaluate the candidate's ability to perform the exact responsibilities mentioned.`;
    ...
    if (setupData.jdText) {
      const positionText =
        setupData.jobPosition
          ? `a **${setupData.jobPosition}** position`
          : 'a position';

      const levelText =
        setupData.level
          ? ` at the **${setupData.level}** level`
          : '';

      const techStacksText =
        setupData.techStacks &&
        setupData.techStacks.length > 0
...
5. **No Repetition**: Do NOT ask generic textbook questions. Ask questions that require the candidate to demonstrate applied understanding of the JD requirements.
6. **Bilingual Output**: Provide each question in both English (content.en) and Vietnamese (content.vi). The Vietnamese version must be a natural translation, not a word-for-word translation.
...
5. **No Repetition**: Do NOT ask generic textbook questions. Ask questions that require applied understanding.
