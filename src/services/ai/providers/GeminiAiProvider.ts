...
import { EvaluationQuestion, generationPrompt, evaluationPrompt, parseProviderJson, validateGeneration, validateEvaluation, validateUsage, outputError } from '../prompt-security';
...
export class GeminiAiProvider implements IAiProvider {
  ...
  private async request(systemInstruction: string, userContent: string) {
    const model = this.genAI.getGenerativeModel({
      model: 'gemini-3.6-flash', systemInstruction,
      generationConfig: { temperature: 0.5, responseMimeType: 'application/json', maxOutputTokens: 8192 },
    });
    // Only transport failures are retried. Policy and schema validation happen after this boundary.
    const result = await providerRetry(signal => model.generateContent({
      contents: [{ role: 'user', parts: [{ text: userContent }] }],
    }, { signal }));
    if (result.response.promptFeedback?.blockReason ||
        result.response.candidates?.length !== 1 || result.response.candidates[0].finishReason !== 'STOP') throw outputError();
    let value: unknown;
    try { value = parseProviderJson(result.response.text()); } catch { throw outputError(); }
    const usage = result.response.usageMetadata;
    const audit = validateUsage({ promptTokenCount: usage?.promptTokenCount ?? 0,
      candidatesTokenCount: usage?.candidatesTokenCount ?? 0, totalTokenCount: usage?.totalTokenCount ?? 0 });
    return { value, audit };
  }
